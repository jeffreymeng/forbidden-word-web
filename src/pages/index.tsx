...
export default function IndexPage(): ReactElement {
  return (
    <Layout showSubtitle>
      ...
      <Row className={"mt-3"}>
        <Col xs={12} md={6}>
          <Button variant={"outline-primary"} className={"mt-3"} block onClick={(): Promise<void> => navigate("/join")}>
            Join Game
          </Button>
        </Col>
        <Col xs={12} md={6}>
          <Button variant={"outline-primary"} className={"mt-3"} block onClick={(): Promise<void> => navigate("/create")}>
            Create Game
          </Button>
        </Col>
      </Row>
